...
uint32_T raptor_GetDeltaTime_us(uint32_T* last)
{
  uint32_T current_ticks = raptor_ticks_us();
  uint32_T delta = 0;
...
uint32_T raptor_GetDeltaTime_ms(uint32_T* last)
{
  uint32_T current_ticks = raptor_ticks();
  uint32_T delta = 0;
  if (*last > current_ticks) {
    delta = (0xFFFFFFFF - *last) + current_ticks + 1;
  } else {
